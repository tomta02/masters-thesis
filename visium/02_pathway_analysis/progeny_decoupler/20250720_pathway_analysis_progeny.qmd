...
act_lst = lapply(act_lst, function(acts) {
  acts = acts[order(acts$spatial_domain),]
})
```
...
plots = mapply(function(mat, nm) {
  ...
  colnames(ann_row) <- "Spatial Domain"
...
for (idx in (seq_along(plots))) {
  ...
  # open png
  png(paste0("/g/saka/Tatjana/data/data_for_testing_stuff/progeny_top500_genes/run_on_sctransformed_mat/heatmap_acts_s", idx, ".png"), 
      width = 1500, 
      height = 2000, 
      res = 300)
