...
trial = act_list_flt[[1]]
trial$spatial_domain = as.factor(trial$spatial_domain)
```

```{r}
# change dataframe shape to long, to plot all pathways simultaneously
trial_long = trial %>%
...
```{r}
# first, check if there are any statisticallt significant differences between pathway activities across spatial domains w kruskal wallis (nonparametric alternative to one-way anova)
stats = trial_long %>%
  group_by(pathway) %>%
...
```{r}
# also want to add count of "non-na" values to each violin plot 
# "trial"/"trial_long" very sparse, since excluded non-significant spots (where there was non-significant relationship between gene expression pattern and certain pathway activity)
...
ggsave("/g/saka/Tatjana/data/data_for_testing_stuff/pathways.png", width = 5, height = 20, dpi = 300)
```

```{r}
p = ggplot(trial_long, aes(x = spatial_domain, y = activity)) +
  geom_violin(trim = FALSE) +
  geom_boxplot(width = 0.1, outlier.shape = NA, alpha = 0.3) +
  geom_text(data = counts,
            aes(x = spatial_domain, y = max(trial_long$activity, na.rm = TRUE) + 0.5,
                label = non_na),
...
ggsave("/g/saka/Tatjana/data/data_for_testing_stuff/pathways_w_stat_sig.png", width = 5, height = 25, dpi = 300)
```

### Now do all of that for all samples:
...
```{r}
# use kruskal wallis test to calc whether there is a sign. difference between pathway activation values actoss spatial domains
stats_all = act_list_flt_long %>%
  map(~ .x %>%
        group_by(pathway) %>%
        kruskal_test(activity ~ spatial_domain)
      )
...
```{r}
# do posthoc test (dunn) to identify which exact domains are sign. different 
posthoc_all = act_list_flt_long %>% 
  map(~ .x %>%
        group_by(pathway) %>%
        dunn_test(activity ~ spatial_domain, p.adjust.method = "BH") %>%
        add_xy_position(x = "spatial_domain")
  ...
  ~ ggplot(.x, aes(x = spatial_domain, y = activity)) +
    ...
    geom_text(data = .y, 
              aes(x = spatial_domain, 
                  y = max(.x$activity, na.rm = TRUE) + 0.5, 
                  label = non_na),
              inherit.aes = FALSE, size = 3) +
    facet_wrap(~ pathway, scales = "free_y", ncol = 1) +
    theme_bw() +
...
for (idx in seq_along(plots_with_stats)) {
  ggsave(paste0("/g/saka/Tatjana/data/data_for_testing_stuff/", idx, ".png"), plots_with_stats[[idx]], width = 5, height = 25, dpi = 300)
}
```
