...
```{r}
# load output files
shared = read.csv("/g/saka/Tatjana/data/01_CNV_analysis/RESULTS_inferCNV_numbat_matching_per_spatial_domain/NEW_spatial_domain_prelim_growth2/LN0025/infercnv_numbat_matching.csv")
shared = shared[,-c(1)]
...
```{r}
numbat = read.csv("/g/saka/Tatjana/data/01_CNV_analysis/RESULTS_inferCNV_numbat_matching_per_spatial_domain/NEW_spatial_domain_prelim_growth2/LN0025/numbat_results.csv")
# gtrellis expects the integer nr. of the chromosome to be preceded by "chr"
numbat$CHROM = paste0("chr", numbat$CHROM)
```

```{r}
sicnv = read.csv("/g/saka/Tatjana/data/01_CNV_analysis/RESULTS_inferCNV_numbat_matching_per_spatial_domain/NEW_spatial_domain_prelim_growth2/LN0025/infercnv_results.csv")
...
names(sicnv_split) = c("diffuse_fdc", "interfollicular_malig", "other")
```

```{r}
# elemeents of sicnv_split are tibbles, convert to df to join
sicnv_split = lapply(sicnv_split, as.data.frame)

# append other dfs to make "master_list"
master_list = c(
  sicnv_split,
  list(sicnv, numbat, shared)
)

names(master_list) = c("diffuse_fdc", "interfollicular_malig", "other", "inferCNV_combined", "numbat", "shared")
```
...
```{r}
pdf("/g/saka/Tatjana/data/01_CNV_analysis/RESULTS_inferCNV_numbat_matching_per_spatial_domain/NEW_spatial_domain_prelim_growth2/LN0025/gtrellis_plot_LN0025.pdf", width = 20, height = 5)
...
gtrellis_layout(
  ...
  n_track = 6, 
  equal_width = FALSE,
  title = "CNVs inferred using inferCNV, numbat, and overlapping CNVs - LN0025", 
  track_ylab = c("shared", "numbat", "inferCNV_all", "diffuse_fdc", "interfollicular_malig", "other"), 
  track_axis = FALSE,
  xlab = "")
