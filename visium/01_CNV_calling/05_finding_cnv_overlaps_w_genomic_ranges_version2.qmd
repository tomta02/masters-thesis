...
```{r}
# # try out: remove 20% of sequence bins with the lowest count frequency for each chromosome
# rmv_lowest_20pct = function(g_ranges) {
#   
#   ## split granges per chrom
#   g_ranges_splt = split(g_ranges, g_ranges@seqnames)
#   
#   for (i in seq_along(g_ranges_splt)) {
#     
#     # for each chr: get the nr of rows that is equal to 10 %
#     chr_gr = g_ranges_splt[[i]]
#     rownr_20pct = floor((length(chr_gr)) * 0.2)
#     # print(rownr_10pct)
#     
#     # now, sort dfs by their bin count frequencies (lowest to highest) - so we can rmv bins with lowest count frequency
#     chr_gr = chr_gr[order(chr_gr$count), ]
#     
#     # now, of the sorted dataframes, remove the first 10% of sequences (i.e. the sequences with the lowest nr of counts)
#     if (rownr_20pct > 0) {
#       chr_gr = chr_gr[-seq_len(rownr_20pct), ]
#     }
#     
#     g_ranges_splt[[i]] = chr_gr
#   }
#     
#   gr_clean = unlist(g_ranges_splt, use.names = FALSE)
#   return(gr_clean)
# }
```
...
```{r}
# Reshape into sparse BED-like format
gr_df_wide = gr_df %>%
  pivot_wider(
    names_from = barcode,
    values_from = state,
...
write.csv(
  gr_df_wide,
  file = "/g/saka/Tatjana/data/data_for_testing_stuff/20250805_spotbased_cnv_calling_script5_LN0040HD_12BV2R_R1.csv",
  row.names = FALSE)
