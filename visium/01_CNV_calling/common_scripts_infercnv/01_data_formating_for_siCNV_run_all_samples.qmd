...
```{r}
# OLD:
#visiumlist_old = qread("/g/saka/Kristy/projects/composite/analysis/rdata/02_10_visium_flt.qs", nthreads = 32)
# NEW:
...
healthy_ln_mtx = qread(
  "/g/saka/Tatjana/data/V1_healthy_human_lymph_node/healthy_ln_sc2_QC_HGNCgenenames.qs",
  nthreads = 32
  )

healthy_annot = read.csv('/g/saka/Tatjana/data/V1_healthy_human_lymph_node/gc_annot_final.csv')
```
...
create_siCNV_annot = function(seu) {
  ...
  Histology = as.character(seu@meta.data$prelim_growth_2)
...
for (id in names(final_subs_cntmtx_list)) {
  ...
  fname = paste0("/g/saka/Tatjana/data/01_CNV_analysis/patient_data_formated_for_running_infercnv/all_pat_countmat_and_annot_new_spatial_domains/", id, "_countmtx.tsv")
  write.table(mtx, 
              fname,
              sep = "\t", 
              row.names = FALSE)
...
for (id in names(joined_annot_list)) {
  ...
  fname = paste0("/g/saka/Tatjana/data/01_CNV_analysis/patient_data_formated_for_running_infercnv/all_pat_countmat_and_annot_new_spatial_domains/", id, "_annot.tsv")
  write.table(mtx, 
              fname,
              sep = "\t",
              col.names = FALSE,
